...
export default function ArchivedNotices() {
    ...
    useEffect(() => {
        ...
        const fetchNoticeDetails = async () => {
            ...
            try {
                ...
                setData(json.data.admissionNotices_connection.nodes || []);
            } catch (err) {
                setError(err);
            } finally {
                setLoading(false);
    ...
    // Filter past notices
    const expiredNotices = data.filter(notice => new Date(notice.endDate) < today);

    // Apply search
    const searchedNotices = expiredNotices.filter(notice =>
